...
/* Control block lifecycle (v2.0.0: mutators take AllocCap; queries are cap-free) */
...
static inline void oo_retain_OoWeakRef(OoWeakRef w) {
  if (w.ctrl) {
    __atomic_fetch_add(&w.ctrl->weak_count, 1, __ATOMIC_ACQ_REL);
  }
}
static inline void oo_release_OoWeakRef(OoWeakRef w) {
  if (w.ctrl) {
    uint32_t _wc = __atomic_fetch_sub(&w.ctrl->weak_count, 1, __ATOMIC_ACQ_REL);
    if (_wc == 1 && __atomic_load_n(&w.ctrl->strong_count, __ATOMIC_ACQUIRE) == 0) {
      free(w.ctrl);
    }
  }
}
